...
clear
%%
load('re347-r1-data.mat');
h = h_logP_T_v_m(:, 1);
p = exp(h_logP_T_v_m(:, 2));
T = h_logP_T_v_m(:, 3);
vol = h_logP_T_v_m(:, 4);
mu = h_logP_T_v_m(:, 5);
...
fprintf('Interpolating functions T(h,p), rho(h,p), and mu(h,p) created successfully.\n');
Tfsave = 1;
Vfsave = 0;
Mfsave = 0;
...
if Tfsave == 1
    ...
    save('re347-r1-Tfunc.mat', "Tfunc_data");
end
...
if Vfsave == 1
    ...
    save('re347-r1-Vfunc.mat', "Vfunc_data");
end
...
if Mfsave == 1
    ...
    save('re347-r1-Mfunc.mat', "Mfunc_data");
end
...
% %% --- D. Use the Functions for Prediction ---
% % Let's test them on a few new points
% h_test = [2.6e6; 2.8e6; 2.9e6]; % Sample enthalpy values in J/kg
% p_test = [850.0; 1500.0; 1800.0]; % Corresponding typical pressures in kPa
% 
% [T_predicted, T_dh, T_dp] = T_func(h_test, p_test);
% [rho_predicted, rho_dh, rho_dp] = vol_func(h_test, p_test);
% [mu_predicted, mu_dh, mu_dp] = mu_func(h_test, p_test);
% 
% fprintf('\n--- Interpolation Examples with Derivatives ---\n');
% for i = 1:length(h_test)
%     fprintf('At h=%.2e J/kg, p=%.1f kPa:\n', h_test(i), p_test(i));
%     fprintf('  - Predicted T: %.2f K (dT/dh=%.3e, dT/dp=%.3e)\n', T_predicted(i), T_dh(i), T_dp(i));
%     fprintf('  - Predicted rho: %.4f kg/m^3 (drho/dh=%.3e, drho/dp=%.3e)\n', rho_predicted(i), rho_dh(i), rho_dp(i));
%     fprintf('  - Predicted mu: %.2e Pa*s (dmu/dh=%.3e, dmu/dp=%.3e)\n', mu_predicted(i), mu_dh(i), mu_dp(i));
% end
...
% Predict vol over the entire grid
...
title(ax_vol, 'Interpolated Density Surface vs. Original Data');
xlabel(ax_vol, 'Enthalpy h (J/kg)');
ylabel(ax_vol, 'Pressure p (Pa)');
zlabel(ax_vol, 'Volume vol (m^3/kg)');
grid(ax_vol, 'on');
view(ax_vol, -45, 20);
legend(ax_vol, 'show');
